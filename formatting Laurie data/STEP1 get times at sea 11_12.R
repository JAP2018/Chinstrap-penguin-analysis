...
for(i in 1:length(master[,1])){
  ...
  if(substr(call_file,80,80)==" "){
    one_file$id <-paste("11_12",substr(call_file,71,71),paste("0",substr(call_file,79,79),sep=""),sep="_")
  }
  else{
    one_file$id <-paste("11_12",substr(call_file,71,71),substr(call_file,79,80),sep="_") 
  }
...
data<-subset(data,data$LAT!=0) #exclude GPS pts with lat = 0 and lon =0 -- errors
data<-subset(data,data$LON!=0)

data$POSIXct <- as.POSIXct(paste(paste(data$DD,data$MM,data$YY,sep="/"),data$TIME),
...
### Add coastlines 
coast = readOGR("C:/Users/Jessica/Desktop/penguin analysis/Coastline_high_res_polygon", 
...
setwd("C:/Users/Jessica/Desktop/penguin analysis/11_12 analysis") 
write.table(trip_info,file="trip_info_11_12_Feb26.csv", sep = ",",row.names = FALSE)