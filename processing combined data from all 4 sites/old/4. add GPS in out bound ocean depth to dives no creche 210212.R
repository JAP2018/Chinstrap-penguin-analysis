...
for (k in 1:4){
  ...
  for(i in trips_canuse){
    ...
    # #get time of max distance from colony 
    # max_time<-as.POSIXct(one_trip$POSIXct[one_trip$dist2colony==max(one_trip$dist2colony)])
    # one_trip$time_2_next<-c(as.numeric(difftime(one_trip$POSIXct[2:length(one_trip[,1])], 
    #                                             one_trip$POSIXct[1:(length(one_trip[,1])-1)], units = "secs")), NA)
    
    #--------------interpolate to 1 GPS point every 1 minute ------------------
    one_trip$POSIXct <- as.POSIXct(one_trip$POSIXct)
    # idt <- seq( from=one_trip$POSIXct[1], to=one_trip$POSIXct[nrow(one_trip)], by="1 min")
    # #i.e. time must be increasing, so we need to do this separately for each trip 
    # ix = pchip(as.numeric(one_trip$POSIXct), one_trip$Lon, as.numeric(idt)) 
    # iy = pchip(as.numeric(one_trip$POSIXct), one_trip$Lat, as.numeric(idt))
    
    # #create new data frame 
    # onemin_gps <- data.frame(idt,ix,iy)
    # colnames(onemin_gps) <- c("interp_time", "interp_Lon","interp_Lat")
    # 
    # onemin_gps$year <- one_tdr$year[1]
    # onemin_gps$id <- one_tdr$id[1]
    # onemin_gps$stage <- one_tdr$stage[1]
    ...
    # #now remove parts w >1h no data for GPS interpolated to 1 min 
    # onemin_gps_no1h<-foreach (z=1:length(win_1h), .combine='rbind') %do% {
    #   timebin_gps<-onemin_gps[as.POSIXct(onemin_gps$interp_time) %within% win_1h[z],]
    # }
    # remain_org_gps<-foreach (z=1:length(win_1h), .combine='rbind') %do% {
    #   timebin_org_gps<-one_trip[as.POSIXct(one_trip$POSIXct) %within% win_1h[z],]
    # }
    # 
    ...
    if(nrow(data_canuse)>0){
      ...
      # dive_w_GPS$in_out<-ifelse(as.POSIXct(dive_w_GPS$start_time)<max_time,"out","in") #max distance considered inbound
      dive_w_GPS <- data.frame(data_canuse,Lon,Lat)
      final[[k]]<-rbind(final[[k]],dive_w_GPS)
      # onemin_gps_clean <- rbind(onemin_gps_clean,onemin_gps_no1h)
      # rows_org_gps <- append(rows_org_gps,nrow(remain_org_gps)) 
...
# write.csv(file = "4.gps interp 1min 1h removed 210212.csv", onemin_gps_clean)

write.csv(file = "4. 4season dive GPS in out 210212.csv", dive_data)
